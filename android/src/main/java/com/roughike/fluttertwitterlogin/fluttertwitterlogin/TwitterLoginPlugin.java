...
public class TwitterLoginPlugin extends Callback<TwitterSession> implements MethodCallHandler, PluginRegistry.ActivityResultListener {
    ...
    private static final String METHOD_GetEmail = "getEmail";
    ...
    public void onMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            ...
            case METHOD_LOG_OUT:
                logOut(result, call);
                break;
                case METHOD_GetEmail:
                getEmail(result, call);
                break;
            default:
    ...
    private void getEmail(final Result result, MethodCall call) {
        Log.d("Email", "success: start" );

        TwitterSession session = TwitterCore.getInstance().getSessionManager().getActiveSession();

        authClientInstance.requestEmail(session, new Callback<String>() {
            @Override
            public void success(final com.twitter.sdk.android.core.Result<String> email) {
            ...
            public void failure(final TwitterException exception) {
                Log.d("Email", "success: " +exception.getMessage());
